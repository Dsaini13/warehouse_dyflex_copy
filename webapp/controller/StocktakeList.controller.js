sap.ui.define([
	...
	return BaseController.extend("dyflex.mm.s4cloud.warehouse.controller.StocktakeList", {
		 ...
		 * @memberOf dyflex.mm.s4cloud.warehouse.view.StocktakeList
		 */
		onInit: function () {
			
			var that = this;
			this._oTable = this.byId("idStocktakeListTable");
			...
			this._oViewModel = new JSONModel({
				...
				saveAsTileTitle: this.getResourceBundle().getText("saveAsTileTitle", this.getResourceBundle().getText("stocktakeListTitle")),
		...
		onUpdateFinished : function (oEvent) {
			...
			if (iTotalItems && oTable.getBinding("items").isLengthFinal()) {
				sTitle = this.getResourceBundle().getText("stocktakeListTableTitleCount", [iTotalItems]);
			} else {
				sTitle = this.getResourceBundle().getText("stocktakeListTableTitle");
			}
		...
		onSearch : function (oEvent) {
			if (oEvent.getParameters().refreshButtonPressed) {
				...
				if (sQuery && sQuery.length > 0) {
					var aFilters = [];
					aFilters.push(new Filter("PhysicalInventoryDocument", FilterOperator.EQ, sQuery));
